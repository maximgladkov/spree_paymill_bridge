module Spree
  class Gateway
    class PaymillBridge < Spree::Gateway
      ...
      def authorize(amount, card, options)
        paymill_request do
          Paymill::Preauthorization.create(amount: amount,
                                           currency: options[:currency],
                                           payment: card.paymill_payment)
        end
      end
      ...
      def purchase(amount, card, options)
        paymill_request do
          Paymill::Transaction.create(amount: amount,
                                      currency: options[:currency],
                                      payment: card.paymill_payment,
                                      client: card.paymill_client)
        end
      end

      def credit(amount, _card, transaction_id, _opts)
        paymill_request do
          Paymill::Refund.create(amount: amount, id: transaction_id)
        end
      end

      def void(authorization, _card, _options = {})
        paymill_request do
      ...
      def create_profile(payment)
        ...
        paymill_request do
          card.paymill_client =
            Paymill::Client.find_or_create(payment.order.email, card.name).id
          card.paymill_payment =
            Paymill::Payment.create(token: card.token, client: card.paymill_client).id
          card.save
        end
      end
